...
int action::_move(int** map, Block* block, int x, int y, int sign) {
...
/* 矩阵旋转90度 */
void action::rotation(Block* block, int director) {
...
void action::_transfer(int** map, Block* block, int sign) {
...
/* 旋转 */
void action::transfer(int** map, Block* block) {
...
//在action类的基础上派生出两个玩家类，以便双人游戏

class player1:public action{
	public:
		int ch = 0;
	    int t = 50;
	    void getAction();
		void action::_transfer(int** map, Block* block); 
		void action::rotation(Block* block, int director);
		// 我们只旋转左上角3*3的矩阵，并且向director旋转90度
		
		int _move(int** map, Block* block, int x, int y, int sign); 
	    //消除所有属于block的块，重新定位block的位置，生成新的block 
		int action::_action(int** map, Block* block, int c) {
	        int isButtom = 0;
	        ch = c;
	        if (ch == -1) {
		        getAction();
